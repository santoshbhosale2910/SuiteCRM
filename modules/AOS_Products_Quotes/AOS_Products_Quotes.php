    ...
    function save_lines($post_data, $parent, $groups = array(), $key = '')
    {
        ...
        for ($i = 0; $i < $line_count; ++$i) {
            ...
            if (isset($post_data[$key . 'deleted'][$i]) && $post_data[$key . 'deleted'][$i] == 1) {
                ...
                if (trim($product_quote->product_id) != '' && trim($product_quote->name) != '') {
                    ...
                    if($post_data['product_priceTables_'.$product_quote->product_id]) {
                        $arrPriceTeable = $post_data['product_priceTables_'.$product_quote->product_id];
                        $product_quote->charge_price_table_c = json_encode($arrPriceTeable);
                    }
                    $product_quote->aos_product_categories_id_c = $post_data[$key . 'category_id'][$i];
